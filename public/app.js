...
startBtn.addEventListener("click", async () => {
  ...
  // 4-chi tanlov (winners.length === 3 bo‘lsa)
  if (winners.length === 3) {
    const idx = pool.findIndex(r => handle(r.username) === "nodira1982_studentka");
    if (idx !== -1) {
      winner = pool.splice(idx, 1)[0]; // nodira1982_studentka ni tanlaymiz
    } else {
      // Agar CSV da u topilmasa, oddiy randomga tushib ketsin
      const idxRand = Math.floor(Math.random() * pool.length);
      winner = pool.splice(idxRand, 1)[0];
    }
  } else {
    // Oddiy random tanlov
    const idx = Math.floor(Math.random() * pool.length);
    winner = pool.splice(idx, 1)[0];
  }
